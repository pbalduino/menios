#include <kernel/apic.h>
#include <kernel/console.h>
#include <kernel/driver.h>
#include <kernel/driver/ps2kb.h>
#include <kernel/driver/ps2.h>
#include <kernel/input/keyboard.h>
#include <kernel/kernel.h>
#include <kernel/tsc.h>
#include <kernel/serial.h>
#include <kernel/idt.h>
#include <kernel/tty.h>

#include <uacpi/acpi.h>
#include <uacpi/tables.h>
#include <uacpi/status.h>

#include <stdbool.h>
#include <stdio.h>
#include <string.h>

#define KB_BUFFER_SIZE 64

#define PIC1_COMM 0x20
#define PIC1_DATA (PIC1_COMM + 1)

#define PIC2_COMM 0xa0
#define PIC2_DATA (PIC2_COMM + 1)

extern void ps2kb_isr_handler();

static volatile uint8_t key_buffer[KB_BUFFER_SIZE];
static volatile uint8_t buffer_head;
static volatile uint8_t buffer_tail;

static bool left_shift;
static bool right_shift;
static bool left_ctrl;
static bool right_ctrl;
static bool left_alt;
static bool right_alt;
static bool caps_lock;
static bool extended_code;
static bool pic_remapped;
static bool ps2kb_started;
static uint32_t ps2kb_debug_events;

static const char scancode_unshift[128] = {
  0,  27, '1', '2', '3', '4', '5', '6', '7', '8',     /* 9 */
  '9', '0', '-', '=', '\b',     /* Backspace */
  '\t',                 /* Tab */
  'q', 'w', 'e', 'r',   /* 19 */
  't', 'y', 'u', 'i', 'o', 'p', '[', ']', '\n', /* Enter key */
  0,                  /* 29   - Control */
  'a', 's', 'd', 'f', 'g', 'h', 'j', 'k', 'l', ';',     /* 39 */
  '\'', '`',   0,                /* Left shift */
  '\\', 'z', 'x', 'c', 'v', 'b', 'n',                    /* 49 */
  'm', ',', '.', '/',   0,                              /* Right shift */
  '*',
  0,  /* Alt */
  ' ',  /* Space bar */
  0,  /* Caps lock */
  0,  /* 59 - F1 key ... > */
  0,   0,   0,   0,   0,   0,   0,   0,
  0,  /* < ... F10 */
  0,  /* 69 - Num lock*/
  0,  /* Scroll Lock */
  0,  /* Home key */
  0,  /* Up Arrow */
  0,  /* Page Up */
  '-',
  0,  /* Left Arrow */
  0,
  0,  /* Right Arrow */
  '+',
  0,  /* 79 - End key*/
  0,  /* Down Arrow */
  0,  /* Page Down */
  0,  /* Insert Key */
  0,  /* Delete Key */
  0,   0,   0,
  0,  /* F11 Key */
  0,  /* F12 Key */
  0,  /* All other keys are undefined */
};

static const char scancode_shift[128] = {
  [0x02] = '!', [0x03] = '@', [0x04] = '#', [0x05] = '$', [0x06] = '%',
  [0x07] = '^', [0x08] = '&', [0x09] = '*', [0x0A] = '(', [0x0B] = ')',
  [0x0C] = '_', [0x0D] = '+', [0x10] = 'Q', [0x11] = 'W', [0x12] = 'E',
  [0x13] = 'R', [0x14] = 'T', [0x15] = 'Y', [0x16] = 'U', [0x17] = 'I',
  [0x18] = 'O', [0x19] = 'P', [0x1A] = '{', [0x1B] = '}', [0x1E] = 'A',
  [0x1F] = 'S', [0x20] = 'D', [0x21] = 'F', [0x22] = 'G', [0x23] = 'H',
  [0x24] = 'J', [0x25] = 'K', [0x26] = 'L', [0x27] = ':', [0x28] = '"',
  [0x29] = '~', [0x2B] = '|', [0x2C] = 'Z', [0x2D] = 'X', [0x2E] = 'C',
  [0x2F] = 'V', [0x30] = 'B', [0x31] = 'N', [0x32] = 'M', [0x33] = '<',
  [0x34] = '>', [0x35] = '?'
};

static inline bool buffer_empty(void) {
  return buffer_head == buffer_tail;
}

static void buffer_push(uint8_t ch) {
  serial_printf("ps2kb: buffer_push start ch=0x%02x head=%u tail=%u\n",
                (unsigned)ch,
                (unsigned)buffer_head,
                (unsigned)buffer_tail);
  uint8_t next = (buffer_head + 1) % KB_BUFFER_SIZE;
  if(next == buffer_tail) {
    serial_printf("ps2kb: buffer_push overflow, dropping tail index %u value=0x%02x\n",
                  (unsigned)buffer_tail,
                  (unsigned)key_buffer[buffer_tail]);
    buffer_tail = (buffer_tail + 1) % KB_BUFFER_SIZE;
  }
  key_buffer[buffer_head] = ch;
  buffer_head = next;
  serial_printf("ps2kb: buffer_push end head=%u tail=%u\n",
                (unsigned)buffer_head,
                (unsigned)buffer_tail);
  stdin_enqueue_char(ch);
}

static uint8_t buffer_pop(void) {
  serial_printf("ps2kb: buffer_pop head=%u tail=%u\n",
                (unsigned)buffer_head,
                (unsigned)buffer_tail);
  uint8_t value = 0;
  if(!buffer_empty()) {
    value = key_buffer[buffer_tail];
    buffer_tail = (buffer_tail + 1) % KB_BUFFER_SIZE;
    serial_printf("ps2kb: buffer_pop value=0x%02x new_tail=%u\n",
                  (unsigned)value,
                  (unsigned)buffer_tail);
  } else {
    serial_printf("ps2kb: buffer_pop empty\n");
  }
  return value;
}

static bool is_alpha(char ch) {
  return (ch >= 'a' && ch <= 'z');
}

static char translate_scancode(uint8_t code) {
  char base = scancode_unshift[code];
  if(base == 0) {
    return 0;
  }

  bool shift = left_shift || right_shift;

  if(is_alpha(base)) {
    bool upper = shift ^ caps_lock;
    if(upper) {
      base = (char)(base - ('a' - 'A'));
    }
    return base;
  }

  if(shift) {
    char shifted = scancode_shift[code];
    if(shifted != 0) {
      return shifted;
    }
  }

  if(caps_lock && (base == ' ')) {
    return base;
  }

  return base;
}

static uint8_t current_modifiers(void) {
  uint8_t mods = 0;
  if(left_shift) {
    mods |= KBD_MOD_LEFT_SHIFT;
  }
  if(right_shift) {
    mods |= KBD_MOD_RIGHT_SHIFT;
  }
  if(left_ctrl || right_ctrl) {
    mods |= KBD_MOD_CTRL;
  }
  if(left_alt || right_alt) {
    mods |= KBD_MOD_ALT;
  }
  if(caps_lock) {
    mods |= KBD_MOD_CAPS_LOCK;
  }
  return mods;
}

static void push_keyboard_event(uint16_t scancode, bool pressed, uint8_t ascii) {
  keyboard_event_t event = {
    .timestamp_ns = ns_from_boot(),
    .scancode = scancode,
    .ascii = ascii,
    .modifiers = current_modifiers(),
    .pressed = pressed ? 1u : 0u,
    .reserved = {0, 0, 0},
  };
  serial_printf("ps2kb: event timestamp=%llu scancode=0x%03x ascii=0x%02x pressed=%u mods=0x%02x\n",
                (unsigned long long)event.timestamp_ns,
                (unsigned)event.scancode,
                (unsigned)event.ascii,
                (unsigned)event.pressed,
                (unsigned)event.modifiers);
  keyboard_device_enqueue(&event);
}

static inline void io_wait(void) {
  outb(0x80, 0);
}

static void pic_remap(void) {
  if(pic_remapped) {
    return;
  }

  uint8_t master_mask = inb(PIC1_DATA);
  uint8_t slave_mask  = inb(PIC2_DATA);

  outb(PIC1_COMM, 0x11); io_wait();
  outb(PIC2_COMM, 0x11); io_wait();

  outb(PIC1_DATA, 0x20); io_wait();
  outb(PIC2_DATA, 0x28); io_wait();

  outb(PIC1_DATA, 0x04); io_wait();
  outb(PIC2_DATA, 0x02); io_wait();

  outb(PIC1_DATA, 0x01); io_wait();
  outb(PIC2_DATA, 0x01); io_wait();

  outb(PIC1_DATA, master_mask); io_wait();
  outb(PIC2_DATA, slave_mask); io_wait();

  pic_remapped = true;
}

void ps2_write_data(uint8_t data) {
  outb(PS2_DATA_PORT, data);
}

void ps2_write_command(uint8_t command) {
  outb(PS2_COMMAND_PORT, command);
}

void ps2_wait_write() {
  while (inb(PS2_COMMAND_PORT) & 0x02);
}

static void irq_eoi(void) {
  outb(PIC1_COMM, 0x20);
}

static inline void keyboard_ack_irq(void) {
  irq_eoi();
  apic_send_eoi();
}

void ps2kb_handler() {
  uint8_t raw = inb(PS2_DATA_PORT);

  ps2kb_debug_events++;
  serial_printf("ps2kb_handler: raw=0x%02x ext=%s head=%u tail=%u event=%u\n",
                raw,
                extended_code ? "pending" : "none",
                (unsigned)buffer_head,
                (unsigned)buffer_tail,
                (unsigned)ps2kb_debug_events);

  if(raw == 0xE0) {
    serial_printf("ps2kb_handler: saw E0 prefix\n");
    extended_code = true;
    keyboard_ack_irq();
    return;
  }

  if(raw == 0xE1) {
    serial_printf("ps2kb_handler: saw E1 prefix\n");
    extended_code = false;
    keyboard_ack_irq();
    return;
  }

  if(raw == 0xFA || raw == 0xFE) {
    serial_printf("ps2kb_handler: raw=0x%02x (ack/resend)\n", raw);
    keyboard_ack_irq();
    return;
  }

  bool is_extended = extended_code;
  extended_code = false;

  bool release = (raw & 0x80) != 0;
  uint8_t code = raw & 0x7F;
  uint16_t scancode = (uint16_t)code | (is_extended ? 0x0100u : 0u);
  uint8_t ascii = 0;

  serial_printf("ps2kb_handler: code=0x%02x extended=%s release=%s\n",
                code,
                is_extended ? "yes" : "no",
                release ? "yes" : "no");

  if(is_extended) {
    switch(code) {
      case 0x1D: // Right Control
        right_ctrl = !release;
        break;
      case 0x38: // Right Alt
        right_alt = !release;
        break;
      case 0x48: // Up Arrow
        if(!release) {
          serial_printf("ps2kb_handler: queue up arrow sequence\n");
          buffer_push('\x1b'); buffer_push('['); buffer_push('A');
        }
        break;
      case 0x50: // Down Arrow
        if(!release) {
          serial_printf("ps2kb_handler: queue down arrow sequence\n");
          buffer_push('\x1b'); buffer_push('['); buffer_push('B');
        }
        break;
      case 0x4B: // Left Arrow
        if(!release) {
          serial_printf("ps2kb_handler: queue left arrow sequence\n");
          buffer_push('\x1b'); buffer_push('['); buffer_push('D');
        }
        break;
      case 0x4D: // Right Arrow
        if(!release) {
          serial_printf("ps2kb_handler: queue right arrow sequence\n");
          buffer_push('\x1b'); buffer_push('['); buffer_push('C');
        }
        break;
      default:
        break;
    }
    serial_printf("ps2kb_handler: extended push event scancode=0x%03x ascii=0x%02x\n",
                  (unsigned)scancode,
                  (unsigned)ascii);
    push_keyboard_event(scancode, !release, ascii);
    keyboard_ack_irq();
    return;
  }

  switch(code) {
    case 0x2A: // Left Shift
      serial_printf("ps2kb_handler: left shift %s\n", release ? "released" : "pressed");
      left_shift = !release;
      push_keyboard_event(scancode, !release, ascii);
      keyboard_ack_irq();
      return;
    case 0x36: // Right Shift
      serial_printf("ps2kb_handler: right shift %s\n", release ? "released" : "pressed");
      right_shift = !release;
      push_keyboard_event(scancode, !release, ascii);
      keyboard_ack_irq();
      return;
    case 0x1D: // Left Control
      serial_printf("ps2kb_handler: left ctrl %s\n", release ? "released" : "pressed");
      left_ctrl = !release;
      push_keyboard_event(scancode, !release, ascii);
      keyboard_ack_irq();
      return;
    case 0x38: // Left Alt
      serial_printf("ps2kb_handler: left alt %s\n", release ? "released" : "pressed");
      left_alt = !release;
      push_keyboard_event(scancode, !release, ascii);
      keyboard_ack_irq();
      return;
    case 0x3A: // Caps Lock
      if(!release) {
        caps_lock = !caps_lock;
        serial_printf("ps2kb_handler: caps lock toggled -> %s\n", caps_lock ? "on" : "off");
      }
      push_keyboard_event(scancode, !release, ascii);
      keyboard_ack_irq();
      return;
    default:
      break;
  }

  if(!release) {
    char ch = translate_scancode(code);
    if(ch != 0) {
      ascii = (uint8_t)ch;
      serial_printf("ps2kb_handler: translated code=0x%02x -> ascii=0x%02x '%c'\n",
                    code,
                    (unsigned)ascii,
                    (ch >= 32 && ch < 127) ? ch : '.');
      buffer_push((uint8_t)ch);
    } else {
      serial_printf("ps2kb_handler: code=0x%02x produced no ascii\n", code);
    }
  } else {
    serial_printf("ps2kb_handler: release code=0x%02x\n", code);
  }

  serial_printf("ps2kb_handler: final push scancode=0x%03x ascii=0x%02x release=%u\n",
                (unsigned)scancode,
                (unsigned)ascii,
                release ? 1u : 0u);
  push_keyboard_event(scancode, !release, ascii);
  keyboard_ack_irq();
}

static bool ps2_read_byte(uint8_t *out) {
  for(int timeout = 0; timeout < 100000; ++timeout) {
    if(inb(PS2_COMMAND_PORT) & 0x01) {
      *out = inb(PS2_DATA_PORT);
      return true;
    }
  }
  return false;
}

void ps2kb_start(void) {
  if(ps2kb_started) {
    return;
  }

  buffer_head = buffer_tail = 0;
  left_shift = right_shift = false;
  left_ctrl = right_ctrl = false;
  left_alt = right_alt = false;
  caps_lock = false;
  extended_code = false;
  ps2kb_debug_events = 0;

  pic_remap();

  struct acpi_fadt *fadt = NULL;
  uacpi_status status = uacpi_table_fadt(&fadt);
  if(uacpi_unlikely_error(status)) {
    serial_printf("ps2kb_start: failed to retrieve FADT (%s)\n", uacpi_status_to_string(status));
    return;
  }

  if((fadt->iapc_boot_arch & ACPI_IA_PC_8042) == 0) {
    serial_printf("ps2kb_start: system does not advertise an 8042 controller\n");
    return;
  }

  // Flush any pending data
  while(inb(PS2_COMMAND_PORT) & 0x01) {
    (void)inb(PS2_DATA_PORT);
  }

  ps2_wait_write();
  ps2_write_command(PS2_DISABLE_FIRST_PORT);
  serial_printf("ps2kb_start: disabled first PS/2 port\n");

  ps2_wait_write();
  ps2_write_command(0x20); // Read controller configuration byte
  uint8_t config = 0;
  if(!ps2_read_byte(&config)) {
    serial_printf("ps2kb_start: timed out reading controller configuration\n");
    return;
  }
  serial_printf("ps2kb_start: controller config=0x%02x\n", config);

  config |= 0x01;   // Enable first port interrupt
  config &= ~(1 << 4); // Ensure first port clock enabled
  serial_printf("ps2kb_start: controller config updated -> 0x%02x\n", config);

  ps2_wait_write();
  ps2_write_command(PS2_WRITE_MODE);
  serial_printf("ps2kb_start: writing controller config\n");
  ps2_wait_write();
  ps2_write_data(config);

  ps2_wait_write();
  ps2_write_command(PS2_ENABLE_FIRST_PORT);
  serial_printf("ps2kb_start: enabled first PS/2 port\n");

  ps2_wait_write();
  ps2_write_data(PS2_ENABLE_SCANNING);
  uint8_t response;
  if(ps2_read_byte(&response) && response != 0xFA) {
    serial_printf("ps2kb_start: unexpected response 0x%x enabling scanning\n", response);
  } else {
    serial_printf("ps2kb_start: scanning enabled response=0x%02x\n", response);
  }

  uint8_t mask = inb(PIC1_DATA);
  serial_printf("ps2kb_start: initial PIC1 mask=0x%02x\n", mask);
  mask &= (uint8_t)~(1u << 1); // ensure legacy PIC still forwards IRQ1
  outb(PIC1_DATA, mask);
  serial_printf("ps2kb_start: updated PIC1 mask=0x%02x\n", mask);

  if(!apic_configure_irq(1, ISR_KEYBOARD, false, false)) {
    serial_printf("ps2kb_start: failed to route keyboard interrupt through IOAPIC\n");
  }

  serial_printf("ps2kb_start: PS/2 keyboard initialised\n");
  ps2kb_started = true;
}

void ps2kb_shutdown(void) {
  ps2_wait_write();
  ps2_write_command(PS2_DISABLE_FIRST_PORT);
  ps2kb_started = false;
}

static uint8_t ps2kb_read(void) {
  uint8_t ch = 0;
  disable_interrupts();
  ch = buffer_pop();
  enable_interrupts();
  serial_printf("ps2kb_read: returned 0x%02x\n", (unsigned)ch);
  return ch;
}

void ps2kb_write(void) {
}

void ps2kb_ioctl(void) {
}

static struct driver_t ps2kb_driver = {
  .hid = "PNP0303",
  .ioctl = ps2kb_ioctl,
  .name = "Standard PS/2 keyboard",
  .read = ps2kb_read,
  .shutdown = ps2kb_shutdown,
  .start = ps2kb_start,
  .write = ps2kb_write,
};

void ps2kb_init(void) {
  serial_printf("ps2kb_init: Registering driver '%s' for HID '%s'\n", ps2kb_driver.name, ps2kb_driver.hid);
  driver_register(&ps2kb_driver);
  ps2kb_start();
}

int kgetchar(void) {
  while(true) {
    uint8_t ch = ps2kb_read();
    if(ch != 0) {
      serial_printf("kgetchar: delivering 0x%02x\n", (unsigned)ch);
      return (int)ch;
    }
    serial_printf("kgetchar: no data, halting CPU\n");
    asm volatile("hlt");
  }
}

int getchar(void) {
  return kgetchar();
}
