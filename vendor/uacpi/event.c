...
)
{
    ...
    if (irq != g_uacpi_rt_ctx.fadt.sci_int) {
        ...
        if (uacpi_unlikely_error(ret)) {
            serial_printf("find_or_create_gpe_interrupt_ctx: free entry: %p\n", entry);
...
static void gpe_release_implicit_notify_handlers(struct gp_event *event)
{
    ...
    while (handler) {
        ...
        serial_printf("gpe_release_implicit_notify_handlers: free handler: %p\n", handler);
...
static void uninstall_gpe_block(struct gpe_block *block)
{
    ...
    if (block->irq_ctx) {
        ...
        if (ctx->gpe_head == UACPI_NULL) {
            ...
            serial_printf("uninstall_gpe_block: free ctx: %p\n", ctx);
    ...
    if (block->events != UACPI_NULL) {
        ...
        for (i = 0; i < block->num_events; ++i) {
            ...
            case GPE_HANDLER_TYPE_NATIVE_HANDLER_RAW:
                serial_printf("uninstall_gpe_block: free native handler: %p\n", event->native_handler);
    ...
    serial_printf("uninstall_gpe_block: free block: %p\n", block);
    uacpi_free(block->registers,
               sizeof(*block->registers) * block->num_registers);
    
    serial_printf("uninstall_gpe_block: free events: %p\n", block->events);
...
)
{
    ...
    serial_printf("uacpi_uninstall_gpe_handler: free native handler: %p\n", native_handler);
