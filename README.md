...
- **Syscalls & Descriptors**: INT 0x80 dispatcher covering `read`, `write`, `open`, `close`, `lseek`, `mmap`, `munmap`, `pipe`, `dup`, `dup2`, `kill`, `signal`, `sigaction`, `sigprocmask`, `sigreturn`, `fcntl`, `yield`, `sleep`, and `exit`; per-process descriptor tables with CLOEXEC, stdin ring buffer, and serial/framebuffer streams (Issues #60, #89, #96, #102, #103)
